...
**关键：两个坏字节的翻转都以"下降沿 nibble 整体读成 0"为主**（`0x04`/`0x08` 的高 4 位
都是 0，而正确的下降沿 nibble 是 `1101`）。这不是随机独立的 bit 错，是**下降沿采样整体
滑掉**的相干签名——data-dependent 边沿抖动把下降沿采样窗口压出裕度（坑点 17/21 的物理
本质）。两个坏字节相距 ~2800 raw 字节，成簇出现在一小段时间窗内（温漂/抖动的瞬时恶化）。

**物理字节误码率 = 2 / 5358550 = 3.7×10⁻⁷**（此程序、112.5M、4-bit）。而 deframe 层因
一个坏字节触发 ~48 个 0xD5 的重解析级联，把"可见异常"放大到 98 字节——但**物理真值就
是 2 个坏字节**，误码是完全可解释的：**下降沿 nibble 采样滑移**，偶发成簇。
...
- **Part C 是真正的物理级对拍**：手推字节 0xD4 与实测 atom 区 99.9982% 吻合。两个层次的
  ...
  重解析级联放大到 98/5357977 = 1.8×10⁻⁵（含 96 个 artifact 0xD5）。故障可解释：
  **下降沿 nibble 采样滑移**（§7.3），偶发成簇，非系统性。
