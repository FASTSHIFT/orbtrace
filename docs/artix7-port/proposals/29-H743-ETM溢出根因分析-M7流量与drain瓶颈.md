...
1. **确证生成速率差**：同一 func_test，分别在 H743 和 F429 上测**单位时间 ETM 字节率**（FPGA 侧字节计数 / 时间），量化 M7 相对 M4 的倍数。
2. **cache 开关对照**：H743 上**关 I-cache**（清 `CCR.IC`）再抓，若溢出显著减少，直接坐实 cache→高吞吐→高分支率这条链。（这是对用户 cache 假设最干净的判决性实验。）
3. **提 TRACECLK 到 100M**：综合 100M MMCM 比特流，重测溢出是否消失。
4. **BB OFF 全量对照**：BB OFF + 停核，看栈能否 15/15。

---

## 7. 结论

用户的判断正确：**问题不在 SI/线材/绝对带宽，而在 M7 与 M4 的架构差异。I/D cache（叠加双发射、零等待执行）让 M7 全速运行 func_test 的密集分支循环，在 branch broadcast 下产生远超 F429 的 trace 字节率；而 H743 这次的 50M 出口带宽反低于 F429 的 84M，中间还多一级窄口 ETF/ATB。生成远大于 drain，ETM FIFO 溢出，溢出撕裂指令流，调用栈随之被打乱。** 修复方向是提高出口带宽（方案 A）或降低生成流量（方案 B/D）或停核无损（方案 C），并用「关 cache 对照实验」作判决性验证。
